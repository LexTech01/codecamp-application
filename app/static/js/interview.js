...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("rescheduleBooking")
    ?.addEventListener("click", rescheduleBooking);
  ...
  async function init() {
    try {
      ...
      if (panel) {
        panel.innerHTML =
          '<div class="card" style="padding:40px;text-align:center"><p style="color:var(--error);margin-bottom:12px">Failed to load available dates.</p><button class="btn btn-primary" onclick="location.reload()">Try Again</button></div>';
      }
    }
  ...
  function renderCalendar() {
    ...
    const availableInMonth = state.availableDates.filter((d) =>
      d.startsWith(monthPrefix),
    ).length;
    if (!availableInMonth) {
      html +=
        '<div class="cal-empty">No availability this month yet. Please check back soon.</div>';
    }
  ...
  async function selectDate(dateStr) {
    ...
    try {
      ...
      if (!data.slots.length) {
        timeSlotsGrid.innerHTML =
          '<p class="text-muted">No slots available for this date.</p>';
        return;
      }
      timeSlotsGrid.innerHTML = data.slots
        .map(
          (s) => `
      ...
      timeSlotsGrid.innerHTML =
        '<p class="text-muted">Failed to load time slots.</p>';
    }
  }
  ...
  async function cancelBooking() {
    const bookingId =
      document.getElementById("cancelBooking")?.dataset.bookingId;
    if (!bookingId) return;
    const ok = await Cellusys.confirm("Cancel your interview?");
    if (!ok) return;
    await Cellusys.fetchJSON(`/api/interview/cancel/${bookingId}`, {
      method: "POST",
    });
    location.reload();
  }
  ...
  async function rescheduleBooking() {
    const bookingId =
      document.getElementById("rescheduleBooking")?.dataset.bookingId;
    if (!bookingId) return;
    const ok = await Cellusys.confirm(
      "Choose a new time? Your current slot will be freed.",
    );
    if (!ok) return;
    await Cellusys.fetchJSON(`/api/interview/cancel/${bookingId}`, {
      method: "POST",
