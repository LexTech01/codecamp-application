...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function validateStep(step) {
    ...
    for (const field of required) {
      const fieldLabel = field.closest('.form-group')?.querySelector('.form-label')?.textContent.trim() || 'This field';
      if (field.type === 'radio') {
        if (checkedRadioGroups.has(field.name)) continue;
        checkedRadioGroups.add(field.name);
        const selected = panel.querySelector(`input[type="radio"][name="${field.name}"]:checked`);
        if (!selected) {
          valid = false;
          setFieldError(field.name, `Please select ${fieldLabel.toLowerCase()}.`);
          panel.querySelectorAll(`input[type="radio"][name="${field.name}"]`).forEach(radio => {
